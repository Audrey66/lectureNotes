...
@pure def allBig[T](P: T=>B @pure, Q: T=>B @pure, R: T=>B @pure, B: T=>B @pure, C: T=>B @pure): Unit = {
  Deduce(
    ...
    Proof(
      ...
      // no individual, cannot use AllE yet
      //for all subproof for conclusion
      4 Let ((random: T) => subproof(
        //implyI to build goal
        5 Subproof(
          6 Assume(P(random) | Q(random)),
          //use NegI to build goal
          7 Subproof(
            8 Assume(R(random)),
            ...
            //try OrE to get get to goal of F
            15 SubProof(
              16 Assume(P(random)),
            ...
            //use AllE with rand for all premise
            //goal: F
          ),
          24 (!R(random)) by NegI(7)
          //goal:P(random) | Q(random)
        ),
        25 (P(random) | Q(random) __>: P(random) | Q(random)) by ImplyI(5)
        //goal: P(random) | Q(random) __>: P(random) | Q(random)
      )),
